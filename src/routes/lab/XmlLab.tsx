...
import { flattenByXmlTag } from '@/lib/tree'
...
function extractTag(line: string): string | null {
  ...
  const tag = match[1]
  if (tag.toLowerCase() === 'xml') return null
  return tag
}
...
export function XmlLab() {
  ...
  const tagMap = useMemo(() => {
    if (!message) return new Map<string, MessageFieldNode>()
    return flattenByXmlTag(message.versions[0].tree)
  }, [message])

  function handleCursorLine(lineContent: string) {
    const tag = extractTag(lineContent)
    if (!tag) return
    const node = tagMap.get(tag)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          value={sampleId}
          onChange={(e) => { setSampleId(e.target.value); setSelectedNode(null) }}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="flex flex-col gap-2">
          ...
          <XmlEditor value={sample.xml} onCursorLine={handleCursorLine} />
