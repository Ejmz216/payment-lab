...
export const sgpiLessonsEs: Record<string, SgpiLessonTranslation> = {
  ...
  'sgpi-funds-state': {
    ...
    blocks: [
      {
        type: 'money-state',
        heading: 'Sigue el valor, no el nombre del mensaje',
        intro: 'Selecciona cada estado y compara dónde está posicionado el valor, cuál evidencia respaldaría esa conclusión y qué sigue sin probarse.',
        badge: 'simplified-model',
        zones: [
          { id: 'customer-a', label: 'CUSTOMER_A', kind: 'party' },
          { id: 'bank-a', label: 'BANK_A', kind: 'agent' },
          { id: 'settlement-layer', label: 'CAPA DE SETTLEMENT', kind: 'infrastructure' },
        ...
        states: [
          {
            id: 'available',
            label: 'Disponible',
            summary: 'CUSTOMER_A puede usar los fondos mantenidos en la cuenta atendida por BANK_A antes de esta operación.',
            position: 'Disponible para CUSTOMER_A en el lado originador.',
            evidence: 'Evidencia autorizada de balance y disponibilidad en el lado originador para esta operación sintética.',
            notProven: 'Aceptación por SGPI o BANK_B, settlement entre participantes y crédito al beneficiario.',
            activeZoneIds: ['customer-a', 'bank-a'],
            tone: 'available',
          },
          {
            id: 'blocked',
            label: 'Bloqueado / reservado',
            summary: 'BANK_A restringió los fondos para esta operación, por lo que CUSTOMER_A no debería poder reutilizar el monto reservado.',
            position: 'Reservado en BANK_A; todavía no liquidado entre participantes.',
            evidence: 'Un evento autoritativo de reserva o retención proveniente del lado originador.',
            notProven: 'Débito definitivo, settlement en la capa interparticipante o crédito a CUSTOMER_B.',
          ...
          },
          {
            id: 'settled',
            label: 'Settled',
            summary: 'La obligación entre participantes fue liquidada en la capa de settlement dentro de este modelo público simplificado.',
            position: 'Movido en la capa interparticipante hacia BANK_B.',
            evidence: 'El evento autoritativo de settlement o la evidencia final definida por material SGPI autorizado.',
            notProven: 'Posting a CUSTOMER_B y disponibilidad en la cuenta del beneficiario.',
          ...
          },
          {
            id: 'credited',
            label: 'Acreditado',
            summary: 'BANK_B registró los fondos en la cuenta beneficiaria para que CUSTOMER_B pueda utilizarlos.',
            position: 'Disponible para CUSTOMER_B en el lado receptor.',
            evidence: 'Evidencia autoritativa de posting o crédito al beneficiario desde la capa institucional receptora.',
            notProven: 'El flujo interno exacto de posting ni el mensaje ISO exacto utilizado por una institución.',
            activeZoneIds: ['bank-b', 'customer-b'],
            tone: 'credited',
          },
        ],
        schemeNote: 'El material público del SGPI respalda la secuencia desde reserva hasta settlement y crédito al beneficiario. Los asientos, eventos internos, versiones de mensaje y evidencias institucionales exactas siguen como TO VERIFY.',
      },
      {
        type: 'callout',
